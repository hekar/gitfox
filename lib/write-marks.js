...
const markdown = require('./writer/markdown');
const json = require('./writer/json');
...
const jsonPath = (folder) => path.resolve(folder, 'README.json');
...
function* writeTree(root, profile, marks) {
  ...
  fs.writeFileSync(jsonPath(folder), json.write(marks));
  fs.writeFileSync(mdPath(folder), markdown.write(marks));
